...
export default function OptionTable({ options, marketOpen, masked = false, canEdit = false, onAdd, onUpdate, onDelete, onRefresh }) {
  const [showModal, setShowModal] = useState(false);
  const [editingOption, setEditingOption] = useState(null);

  const sorted = [...options].sort((a, b) => (b.pct_of_portfolio || 0) - (a.pct_of_portfolio || 0));
    ...
    <div className="mb-8">
      ...
      {options.length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center text-gray-500">
          {canEdit ? 'No options yet. Click "Add Option" to get started.' : 'No options to display.'}
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
                ...
                {sorted.map(o => {
                  ...
                  return (
                    <tr key={o.id} className="border-b border-gray-800/50 hover:bg-gray-900/50 transition">
                      ...
                      <td className="py-3 pr-4 text-right tabular-nums">{masked ? <Masked /> : formatCurrency(o.strike)}</td>
                      <td className="py-3 pr-4 text-gray-300">{o.expiration}</td>
                      <td className="py-3 pr-4 text-right tabular-nums">{masked ? <Masked /> : o.contracts}</td>
                      <td className="py-3 pr-4 text-right tabular-nums font-medium">{masked ? <Masked /> : formatCurrency(o.current_price)}</td>
                      <td className="py-3 pr-4 text-right tabular-nums font-medium">{masked ? <Masked /> : formatCurrency(o.market_value)}</td>
                      <td className="py-3 pr-4 text-right tabular-nums text-gray-400">{(o.pct_of_portfolio ?? 0).toFixed(1)}%</td>
              ...
              <tfoot>
                <tr className="text-gray-300 font-medium">
                  ...
                  <td className="pt-3 pr-4 text-right tabular-nums">{masked ? <Masked /> : formatCurrency(totalValue)}</td>
